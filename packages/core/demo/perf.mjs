// Headless perf harness for the @shipgraph/core demo. Drives window.__demo
// (the PUBLIC API) at 1x and 5x, sampling render fps/frame-time during the
// active physics phase and after settle, exercising hover halo + eased focus
// + filters + collapse. Writes perf-results.json + screenshots.
...
const URL = process.env.DEMO_URL || 'http://localhost:5190/';
...
async function measure(scale) {
  ...
  // interaction under load: hover halos + eased focus camera
...
// sanity: exercise filters + collapse once so the harness proves they run
